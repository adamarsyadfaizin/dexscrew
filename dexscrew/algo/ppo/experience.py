...
class ExperienceBuffer(Dataset):
    def __init__(self, num_envs, horizon_length, batch_size, minibatch_size,
                 obs_dim, act_dim, priv_dim, point_cloud_sampled_dim, device, proprio_dim=32*30,proprio_length=30):
        self.device = device
        self.num_envs = num_envs
        self.transitions_per_env = horizon_length
        self.priv_info_dim = priv_dim
        self.point_cloud_sampled_dim = point_cloud_sampled_dim
        ...
        self.proprio_length = proprio_length
        ...
        self.storage_dict = {
            ...
            'point_cloud_info': torch.zeros((self.transitions_per_env, self.num_envs, self.point_cloud_sampled_dim, 3), dtype=torch.float32, device=self.device),
            'proprio_hist': torch.zeros((self.transitions_per_env, self.num_envs, self.proprio_length, self.proprio_dim), dtype=torch.float32, device=self.device),
    ...
    def __getitem__(self, idx):
        ...
        return input_dict['values'], input_dict['neglogpacs'], input_dict['advantages'], input_dict['mus'], \
            ...
            input_dict['point_cloud_info'], input_dict['proprio_hist']
